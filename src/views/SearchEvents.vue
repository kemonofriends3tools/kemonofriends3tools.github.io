        ...
        <div class="table-attached-header table-attached-header-solidBottom">
          ...
          <div class="table-attached-header-contents pl-1" style="width: calc(100% - 25px)">
            ...
            <b-row class="w-100">
              ...
              <b-col
                cols="12"
                :xl="getSearchFilterTagXLLayout"
                class="px-1 my-1"
                v-if="SearchFilter.tags.visible"
              >
                <v-select
                  multiple
                  v-model="SearchFilter.tags.value"
                  :options="getTagList"
  ...
  data() {
    return {
      ...
      SearchFilter: {
        ...
        tags: { visible: false, value: [], placeholder: 'タグ' },
      },
  ...
  computed: {
    filteredAttributes() {
      ...
      if (!this.SearchFilter.target.value) {
        //targetが選択されていない場合はmasterAttributesをそのまま返す。
        return tmpAttr;
      } else {
        ...
        //タグ。選択項目に対するand条件。
        if (0 < this.SearchFilter.tags.value.length) {
          tmpAttr = tmpAttr.filter(i =>
            this.SearchFilter.tags.value.every(j => i.customData.tags.some(k => k == j))
          );
        }
    ...
    getNameList() {
      ...
      //tagsの有無によって処理を切り替える。(単純にfilteredAttributesからlistを作り出すとnameを再選択するときに同じ条件のものしか選べなくなる為)
      if (0 < this.SearchFilter.tags.value.length) {
        //tag指定がある場合はfilteredAttributesが既にtagsで絞り込まれている為、filteredAttributesを走査して収集する。
    ...
    getTagList() {
      ...
      //カテゴリーが無いならなにかがおかしい。空配列を返す。
      if (!tmpCategoryValue) return [];

      //カテゴリーの選択項目が特定アイテムかどうか確認（検索対象が判る）
      if (this.originalColumn.has(tmpCategoryValue)) {
        ...
        for (const [iKey, iArray] of this.originalColumn) {
          if (iKey == tmpCategoryValue) {
            ...
            //選択カテゴリーがspecialTagMapに登録されているならその情報をOKリストに追加
            ...
            //非対象カテゴリー。結合文字列、その他文字列共に全てNGリストへ追加。(結合文字列の存在チェックはする。)
      ...
      if (this.originalColumn.has(tmpCategoryValue)) {
        ...
        if (outputArray.length && tmpSet.size) outputArray.push('━━━━━━━━━━');
        //その他タグを入れる
        tmpSet.forEach(i => outputArray.push(i));
      } else {
        //イベント検索。その他タグが先。
        //その他タグを入れる
        tmpSet.forEach(i => outputArray.push(i));
    ...
    //SearchFilterのtags欄のXL時レイアウト値を返す。
    getSearchFilterTagXLLayout() {
      return this.SearchFilter.target.value == 'イベント検索' ? 6 : 12;
    },
    ...
    getPopoverColumns() {
      ...
      if (!this.SearchFilter.category.value) {
        ...
        //tagが選択されいているか調べる。
        //this.SearchFilter.tags.valueは配列が期待される変数だが、空文字にはなってもnullになることはない模様。なので単にサイズで比較する。
        if (0 < this.SearchFilter.tags.value.length) {
          //tag指定有り
          ...
          //tagsを走査
          for (const i of this.SearchFilter.tags.value) {
  ...
  methods: {
    ...
    selectedCategory() {
      ...
      this.SearchFilter.name.visible = this.masterCategoryList
        ...
        ? true
        : false;
      this.SearchFilter.tags.visible = this.SearchFilter.category.value ? true : false;
