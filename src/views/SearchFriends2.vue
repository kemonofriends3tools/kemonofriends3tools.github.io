      ...
      >
        ...
        <template v-slot:table-row="props">
          ...
          <template v-else-if="props.column.label == '回避'">
            <text-highlight :queries="getGlobalSearchTermArray">
...
export default {
  ...
  computed: {
    ...
    //検索文字列を半角または全角スペースでsplitした配列を返す。
    //単純なsplitだが複数個所で利用しているのでキャッシュの効くcomputedとして提供する。
    getGlobalSearchTermArray() {
      //全角スペースを直で書くとlintでエラーになるので文字コードで指定する。
      return this.globalSearchTerm.split(/[\x20\u3000]/);
    },
  },
  methods: {
    ...
    globalSearch(row, col, cellValue) {
      ...
      return this.getGlobalSearchTermArray.every(i => tmpCellString.indexOf(i) != -1);
    },
